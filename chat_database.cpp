...
// 在`user`中，将群聊string2加入到用户string1的群聊字符串中
void ChatDataBase::my_database_user_add_group(string user_name, string group_name)
{
	...
	MYSQL_RES* res = mysql_store_result(mysql);
	MYSQL_ROW row = mysql_fetch_row(res);
	if (row[0] != NULL) // 原来群聊字符串不为空，在末尾加上"|group_name"
	{
		all_group += row[0];
		all_group += "|";
		all_group += group_name;
	}
	else //原来群聊字符串为空，该群聊为第一个群聊，直接加上
	{
		all_group += group_name;
	}

	memset(sql, 0, sizeof(sql));
	sprintf(sql, "update %s set chatgroup = '%s';", user_name.c_str(), all_group.c_str()); // 更新chatgroup
	if (mysql_query(mysql, sql) != 0)
	{
		cout << "mysql_query error" << endl;
	}
